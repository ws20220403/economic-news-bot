...
자동 실행은 한국시간 **06:40~07:50** 구간에 5분 간격으로 점검하며, 가장 먼저 가드를 통과한 실행이 그날 한 번만 발송합니다. GitHub 스케줄러는 종종 몇 분씩 지연되기 때문에, 목표 시각 06:45 부근에 여러 번 시도해 사실상 매일 발송되도록 했습니다. (하루에 한 번만 발송되도록 발송 성공 시 그날 표식을 남깁니다.)
